...
const Home = () => {
    ...
    */
    const handleQuery = async (apiCall, params, isComplexSearch = false) => {
        // 有效的 queryType 列表
        const validQueryTypes = ["destinedCourse", "selectiveCourse", "searchedRecord"];
        if (
            !searchTerm &&
            !validQueryTypes.includes(params.queryType) &&
            !isComplexSearch
        ) {
            alert('查詢內容不可為空');
            return;
    ...
    return (
        <div id="returnPlace">
            ...
            <div id="face">
                ...
                <div id="face-right">
                    {/* 單一按鈕提交 */}
                    {['destinedCourse', 'selectiveCourse', 'searchedRecord'].map((queryType) => (
                        <form id="queryForm" key={queryType}>
                            <div
                                className="query-button"
                                onClick={() => handleQuery(queryByType, filterParams({ queryType }))}
                                role="button"
                                tabIndex={0}
                            >
                                {queryType === 'destinedCourse' && '當期預排'}
                                {queryType === 'selectiveCourse' && '科系選修'}
                                {queryType === 'searchedRecord' && '歷史搜尋'}
                            </div>
                        </form>
                    ))}
                </div>
